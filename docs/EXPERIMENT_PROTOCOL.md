...
- Seeds: 0–4, totaling 180 training runs.
...
For either mixup method, one `lambda ~ Beta(0.2, 0.2)` tensor and one batch permutation are reused for representation and action target. Noise BC adds Gaussian noise with standard deviation `.05` in normalized-state units during training.
